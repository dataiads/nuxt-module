...
if (props.paginationNavigationAction === 'one-by-one') {
  ...
  actionHandlerRight = () => {
    if (props.filter.page.value < pageCount.value) {
      props.filter.page.value += 1;
    }
  }
}
...
if (props.paginationNavigationAction === 'first-last') {
  ...
  actionHandlerRight = () => {
    props.filter.page.value = pageCount.value;
  }
}
...
<template>
  <nav :class="props.class">
    <div v-if="props.filter.page.value > 1" @click="actionHandlerLeft" alt="go to previous page">
    ...
    <div v-if="props.filter.page.value < pageCount" @click="actionHandlerRight" alt="go to next page">
