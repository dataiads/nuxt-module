...
export const useRecommender = (options: UseRecommenderOptions) => {
  ...
  const fetchParams: Record<string, string | number | Ref<string> | Ref<number> | Ref<boolean> | ComputedRef<string> | ComputedRef<number> | ComputedRef<boolean>> = {
  ...
  const _fetcher = useFetch(() => '/api/recommendations/default/' + options.endpoint, {
    params: fetchParams,
    ...(options.fetchOptions ?? {})
  })

  // fetcher post processing (grouping, local pagination...)
  const fetcher: RecommenderResults = {
    data: computed(() => {
      const rawData = _fetcher.data.value
      if (!rawData) {
        return rawData
      }
  ...
  const criteriaValuesCache: Record<string, any> = {}
